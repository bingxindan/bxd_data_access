...
import (
	...
	"github.com/bingxindan/bxd_data_access/framework/provider/log/services"
	"io"
	"strings"
)

type BxdLogServiceProvider struct {
...
// Register 注册一个服务实例
func (l *BxdLogServiceProvider) Register(c framework.Container) framework.NewInstance {
	if l.Driver == "" {
		tcs, err := c.Make(contract.ConfigKey)
		if err != nil {
			// 默认使用console
			return services.NewBxdConsoleLog
		}
	...
	switch l.Driver {
	case "single":
		return services.NewBxdSingleLog
	case "rotate":
		return services.NewBxdRotateLog
	case "console":
		return services.NewBxdConsoleLog
	case "custom":
		return services.NewBxdCustomLog
	default:
		return services.NewBxdConsoleLog
	}
}
...
// Boot 启动的时候注入
func (l *BxdLogServiceProvider) Boot(c framework.Container) error {
	return nil
}

// IsDefer 是否延迟加载
func (l *BxdLogServiceProvider) IsDefer() bool {
	return false
}

// Params 定义要传递给实例化方法的参数
func (l *BxdLogServiceProvider) Params(c framework.Container) []interface{} {
	// 获取configService
...
// Name 定义对应的服务字符串凭证
func (l *BxdLogServiceProvider) Name() string {
